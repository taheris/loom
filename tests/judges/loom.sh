...
test_scratchpad_partial_clarity() {
  ...
  judge_criterion \
    "partial/scratchpad.md tells the agent that the scratchpad is agent-lifecycle-only — the file is created at session start, removed at session end on every exit path, and is a compaction-recovery aid rather than durable storage. It must direct anything that needs to outlive the session to the phase-authorized durable destination instead of the scratchpad, giving concrete examples such as spec/index markdown, bead state or implementation notes, finding evidence/output protocol for inspection-only phases, commit messages, companion docs, or the authorized work queue. The partial directs the agent to move durable-worthy thoughts BEFORE session end so a future agent can find the durable record through the phase's normal surface. Vague guidance like 'write important things down' without naming durable destination categories is insufficient."
}
...
judge_sibling_spec_editing_documents_split() {
  ...
  judge_criterion \
    "partial/sibling_spec_editing.md establishes three things, all in one place: (1) the anchor/sibling editing model — labels passed to loom plan are anchors only, and any spec under specs/ may be edited in-place during the interview when the touched set emerges; (2) the new-sibling-spec carve-out — the planner may decide a section warrants its own spec, create specs/<label>.md, and add the row to docs/README.md, while explicitly saying planning does NOT allocate a bead or epic because loom todo creates spec/work epics later; and (3) the commit-discipline rule — planning sessions edit specs in place but do NOT commit; soft signals like 'looks good' or 'next' or 'accept' authorize the next interview step but never authorize a commit; commits require unambiguous language such as 'commit', 'ship it', 'land the changes', 'land the plane', or 'push it'. The same discipline applies to git push, wrix beads push, and any shared-state mutation. The partial must name all three: the editing model, the no-bead/no-epic planning rule for new sibling specs, and the commit-discipline rule with explicit examples of soft signals vs. unambiguous triggers. Vague phrasing like 'be careful with commits' is insufficient — the partial must enumerate concrete trigger phrases."
}
