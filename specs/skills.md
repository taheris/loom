...
- Exact Pi and Claude native registrar mechanics (config files, package
  directories, RPC commands, version probes) are backend implementation details.
  The user-visible policy is decided: a backend is native-capable only when Loom
  has a concrete, tested registrar for that backend/runtime version; otherwise
  Loom uses prompt disclosure, and Direct remains prompt-disclosure-only.
- Checker-specific `loom-case` schemas and fixture validators are locked one
  checker at a time during implementation, under the strict TOML/no-script rules
  above.
