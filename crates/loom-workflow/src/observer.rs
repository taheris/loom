...
//!
//! `llm` ships `DoomLoopObserver` + `DuplicateResultObserver` so
...
mod tests {
    ...
    use loom_events::identifier::{BeadId, SessionId, ToolCallId};
    use serde_json::json;

    fn envelope(seq: u64) -> EventEnvelope {
        EventEnvelope {
            session_id: SessionId::new("sess-observer").unwrap(),
