...
use loom_driver::state::{ActiveMolecule, BdUpdateFn, CacheDb, CacheError};
...
fn cache_db_init_creates_tables() -> Result<()> {
    ...
    assert_eq!(
        meta,
        vec![vec!["schema_version".to_string(), "8".to_string()]]
    );
    Ok(())
}
...
fn cache_db_open_migrates_v6_drops_current_molecule() -> Result<()> {
    ...
    assert_eq!(
        version,
        vec![vec!["8".to_string()]],
        "schema_version must advance to 8",
    );
...
fn cache_db_open_migrates_v1_to_v2() -> Result<()> {
    ...
    assert_eq!(meta, vec![vec!["8".to_string()]]);
...
fn open_wipes_legacy_todo_cursor_meta_keys() -> Result<()> {
    ...
    assert_eq!(version, vec![vec!["8".to_string()]]);
...
fn todo_resolution_is_single_query_with_invariant_violation_refusal() -> Result<()> {
    ...
    match err {
        CacheError::DuplicateSpecMolecules { label, ids } => {
