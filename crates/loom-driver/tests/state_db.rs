...
#[test]
fn cache_db_init_creates_tables() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
    let _db = CacheDb::open(&db_path)?;
    assert!(db_path.exists(), "cache.db should be created");
    ...
    assert!(
        ...
        "current_molecule must not exist on a fresh v8 schema: {names:?}",
    );
    ...
    assert_eq!(
        meta,
        vec![vec!["schema_version".to_string(), "9".to_string()]]
    );
    Ok(())
}

#[test]
fn cache_db_rebuild_populates_specs_and_molecules() -> Result<()> {
    let dir = tempfile::tempdir()?;
    ...
    let mol = db
        .work_epic(&MoleculeId::new("lm-alpha").unwrap())?
        .context("work epic should be present after rebuild")?;
    assert_eq!(mol.epic_id.as_str(), "lm-alpha");
    assert_eq!(mol.base_commit.as_deref(), Some("abc123"));
    assert_eq!(mol.iteration_count, 0);

    assert!(db.spec_epic(&SpecLabel::new("beta").unwrap())?.is_none());
    Ok(())
}

#[test]
fn cache_db_rebuild_companions() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    write_spec(
        workspace,
...
#[test]
fn cache_db_rebuild_resets_counters() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    write_spec(workspace, "alpha", "# alpha\n")?;
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
    ...
    let mol = db
        .work_epic(&MoleculeId::new("lm-alpha").unwrap())?
        .context("work epic still present after rebuild")?;
    assert_eq!(mol.iteration_count, 0);
    Ok(())
}

/// `CacheDb::open` on a fresh DB must NOT carry the obsolete
...
fn current_molecule_table_does_not_exist() -> Result<()> {
    ...
    assert!(
        rows.is_empty(),
        "current_molecule table must not exist on a v8 schema: {rows:?}",
    );
    Ok(())
}
...
#[test]
fn cache_db_open_migrates_v6_drops_current_molecule() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
    ...
    assert_eq!(
        version,
        vec![vec!["9".to_string()]],
        "schema_version must advance to 9",
    );
    ...
    assert!(
        surviving.is_empty(),
        "current_molecule table must be dropped after v6→v8 migration: {surviving:?}",
    );
...
#[test]
fn cache_increment_iteration_returns_updated_count() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    write_spec(workspace, "alpha", "# alpha\n")?;
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
...
#[test]
fn cache_set_and_reset_iteration_round_trip() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    write_spec(workspace, "alpha", "# alpha\n")?;
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
...
#[test]
fn cache_db_open_migrates_v1_to_v2() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
...
#[test]
fn cache_db_open_is_idempotent_after_migration() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
...
#[test]
fn cache_corruption_recovery() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
    std::fs::write(&db_path, b"this is not a sqlite database\x00\x01\x02")?;

    if CacheDb::open(&db_path).is_ok() {
...
fn notes_cascade_on_spec_delete() -> Result<()> {
    ...
    // Drop the CacheDb handle so its connection lock cannot collide with
    // the side-channel connection below. The cascade depends on
    // PRAGMA foreign_keys = ON, which CacheDb::open already enables for
...
fn open_wipes_legacy_todo_cursor_meta_keys() -> Result<()> {
    ...
    assert_eq!(version, vec![vec!["9".to_string()]]);
...
/// with [`CacheError::DuplicateSpecEpics`] naming every conflicting id,
...
fn todo_resolution_is_single_query_with_invariant_violation_refusal() -> Result<()> {
    ...
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
    let molecules = ["lm-aaa", "lm-bbb"].map(|id| {
        RebuildEpic::Spec(SpecEpicRow {
            epic_id: MoleculeId::new(id).unwrap(),
    ...
    match err {
        CacheError::DuplicateSpecEpics { label, ids } => {
        ...
        other => return Err(anyhow!("expected DuplicateSpecEpics, got {other:?}")),
    }
    Ok(())
}
...
#[test]
fn cache_rebuild_cross_checks_spec_index_and_files() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    ...
    std::fs::write(
        ...
        "| [alpha.md](../specs/alpha.md) | x | lm-a | p |\n",
    )?;
    std::fs::write(workspace.join("specs/beta.md"), "# beta\n")?;
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
    let err = db.rebuild(workspace, &[]).unwrap_err();
    assert!(matches!(err, CacheError::SpecIndexMismatch { .. }));
    Ok(())
}

#[test]
fn cache_rebuild_requires_one_spec_epic_per_indexed_spec() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    std::fs::create_dir_all(workspace.join("docs"))?;
    std::fs::create_dir_all(workspace.join("specs"))?;
...
#[test]
fn cache_db_rebuild_populates_specs_epics_and_companions() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    std::fs::create_dir_all(workspace.join("docs"))?;
    std::fs::create_dir_all(workspace.join("specs"))?;
...
#[test]
fn cache_rebuild_resets_work_epic_counters() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let workspace = dir.path();
    write_spec(workspace, "alpha", "# alpha\n")?;
    let db = CacheDb::open(workspace.join(".loom/cache.db"))?;
...
#[test]
fn cache_rebuild_rolls_back_partial_replacement_on_duplicate_work_ids() -> Result<()> {
    let directory = tempfile::tempdir()?;
    write_spec(directory.path(), "alpha", "# alpha")?;
    let db = CacheDb::open(directory.path().join("cache.db"))?;
    let id = MoleculeId::new("lm-work")?;
    let label = SpecLabel::new("alpha")?;
    let mut epics = epic_fixture(id.clone(), label.clone(), Some("anchor".into()))?.to_vec();
    db.rebuild(directory.path(), &epics)?;
    db.set_iteration(&id, 7)?;
    db.notes_add(&label, "implementation", "keep on failure", 1)?;
    epics.push(epics[1].clone());
    assert!(db.rebuild(directory.path(), &epics).is_err());
    assert_eq!(
        db.work_epic(&id)?
            .context("preserved work epic")?
            .iteration_count,
        7
    );
    assert_eq!(db.notes_list(Some(&label), None)?.len(), 1);
    Ok(())
}

#[test]
fn cache_rebuild_rejects_empty_present_index() -> Result<()> {
    let directory = tempfile::tempdir()?;
    write_spec(directory.path(), "alpha", "# alpha")?;
    std::fs::create_dir(directory.path().join("docs"))?;
    std::fs::write(directory.path().join("docs/README.md"), "# empty index")?;
    let db = CacheDb::open(directory.path().join("cache.db"))?;
    assert!(matches!(
        db.rebuild(directory.path(), &[]),
...
#[test]
fn cache_corruption_recovery_never_implies_clean_todo() -> Result<()> {
    let dir = tempfile::tempdir()?;
    let db_path = dir.path().join("cache.db");
