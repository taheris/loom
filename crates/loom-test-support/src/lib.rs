...
//!
//! Per `specs/tests.md` (Architecture / Property-Based Testing), property
...
/// Shared property-test configuration with an environment override.
pub fn proptest_config() -> ProptestConfig {
    let cases = std::env::var("PROPTEST_CASES")
        .ok()
        .and_then(|raw| raw.parse::<u32>().ok())
        .filter(|cases| *cases > 0)
        .unwrap_or(CI_PROPTEST_CASES);
