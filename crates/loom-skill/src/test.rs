...
mod tests {
    ...
    use crate::source::{SkillProvenance, SkillSource, SourceHash, SourceShape};
    ...
    fn loose_named(name: &str, source: SkillSource, metadata: &str) -> NamedSkill {
        ...
        let provenance = SkillProvenance {
            ...
            built_in_name: None,
            source_hash: SourceHash::new(blake3::hash(markdown.as_bytes()).to_hex().to_string())
    ...
    fn skill_registry_typestate_prevents_misuse() {
        let raw = RawSkillDocument::new(
            ...
            SkillProvenance {
                ...
                built_in_bundle: None,
                built_in_name: None,
                source_hash: SourceHash::new(blake3::hash(b"typed-skill").to_hex().to_string())
                    .expect("valid source hash"),
            },
        );
    ...
    fn skill_frontmatter_diagnostics_by_source() {
        ...
        write(
            repo.path().join("malformed/skill.md").as_path(),
            "---\nname: malformed\ndescription: Invalid unknown field.\nunknown: [unterminated\n---\n",
        );
        let malformed_report =
            discover_workspace(repo.path(), &[PathBuf::from("malformed/skill.md")])
                .expect("auto-discovered malformed YAML warns");
        assert_eq!(malformed_report.set().skills().len(), 0);
        assert_eq!(
            malformed_report.warnings()[0].kind,
        ...
        let malformed_configured = repo.path().join("malformed-configured.md");
        write(
            &malformed_configured,
            "---\nname: malformed\ndescription: Invalid unknown field.\nunknown: [unterminated\n---\n",
        );
        let err = load_configured_paths(repo.path(), &[PathBuf::from("malformed-configured.md")])
            .expect_err("configured malformed YAML errors");
        assert!(matches!(
            err,
            DiscoveryError::InvalidSkill {
                skill_source: SkillSource::Configured,
        ...
        let malformed_override_repo = workspace();
        write(
            &malformed_override_repo
                .path()
                .join(".loom-override/skills/malformed.md"),
            "---\nname: malformed\ndescription: Invalid unknown field.\nunknown: [unterminated\n---\n",
        );
        let err = load_overrides(malformed_override_repo.path())
            .expect_err("override malformed YAML errors");
        assert!(matches!(
            err,
            DiscoveryError::InvalidSkill {
                skill_source: SkillSource::Override,
