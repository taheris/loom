...
This phase is **inspection-only**: under no circumstance do you mutate
`bd` state before emitting your terminal marker. Every concern reaches
bead state through the streamed `LOOM_FINDING:` lines that the driver
consumes; your only signal to the gate is the marker itself.
...
- `LOOM_CLARIFY` — You cannot complete the walk and have a specific
  ...
  Do not write notes or apply the `loom:clarify` label yourself — the
  driver applies the label as part of the mint.
