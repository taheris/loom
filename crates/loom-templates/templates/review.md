...
the mint pipeline **only** through the `evidence` field of a
`route="clarify"` finding line — the driver's mint step lifts
it into the minted clarify bead's description. Do not try to persist
review options yourself with `bd` commands. If the canonical block
