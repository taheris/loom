...
3. **Per bead** — For each bead in turn:
   ...
   6. **Write** the resolution note and apply the bd-state transition.
      ...
      the resolution with `bd update <id> --notes "..."`, then remove
      the msg-queue label with `bd update <id>
      --remove-label=loom:clarify` (or `--remove-label=loom:blocked`)
      so the bead re-enters the work queue for the next implementing
      session. If the resolution closes the work outright (no further
      implementation needed), use `bd close <id>` instead — the driver
      does NOT reverse the close.
...
--remove-label=loom:clarify` / `--remove-label=loom:blocked`), and
optional close (`bd close <id>`) all happen from inside the session.
