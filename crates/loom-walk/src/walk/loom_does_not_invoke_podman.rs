...
//! directly. Both agent backends drive the wrapper via `wrix spawn`
...
//! `wrix spawn` argv; this walk catches the negative half of that
...
const RULE: &str =
    "loom_does_not_invoke_podman — drive containers through `wrix spawn`, never `podman` directly";
            ...
            for needle in &needles {
                if line.contains(*needle) {
                    violations.push(format!(
                        "{}:{} `{}` — spawn through `wrix spawn`, never `podman` directly",
                        rel_path,
                        lineno + 1,
                        needle,
                    ));
